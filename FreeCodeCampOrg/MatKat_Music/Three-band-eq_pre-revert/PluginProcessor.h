...
// This is not explained in the tutorial
#include <array>
template<typename T>
struct Fifo
{
    void prepare(int numChannels, int numSamples) // prepare with channels
    {
        ...
        for( auto& buffer : buffers)
        {
            ...
            buffer.clear(); // Clear the buffer once it's been organised
        }
    }
    
    void prepare(size_t numElements) // prepare with number of elements
    {
...
// This is also not explained in the tutorial
...
using Filter = juce::dsp::IIR::Filter<float>; // IIR Filter
...
private:
    ...
    // Create oscilator to test FFT accuracy
    // Test oscilator =====-----
